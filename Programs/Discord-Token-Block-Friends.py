        ...
        for friend in friends:
            try:
                ...
                if response.status_code == 204 or response.status_code == 200:
                    print(f"{SUCCESS} Status:{red} Blocked {white}| Friend:{red} {friend['user']['username']}", reset)
                else:
                    print(f"{ERROR} Status:{red} Failed  {white}| Friend:{red} {friend['user']['username']}", reset)
            except:
                print(f"{ERROR} Status:{red} Error   {white}| Friend:{red} {friend['user']['username']}", reset)
